...
from mindspore.mint.optim.adamw import (_run_optim_adamw_amsgrad_opt,
    ...
    def construct(self, gradients):
        ...
        for group_id, group in enumerate(self.param_groups):
            ...
            if group.get("amsgrad"):
                self.hyper_map(
                    F.partial(_run_optim_adamw_amsgrad_opt, self.adamw_opt,
                              beta1, beta2, float(lr), group.get("eps"),
                              group.get("weight_decay"), self.state_step,
                              group.get("amsgrad"), maximize),
                    self.master_parameters[start_id:end_id], grads,
                    self.exp_avg[start_id:end_id],
                    self.exp_avg_sq[start_id:end_id],
                    group.get("max_exp_avg_sq"))
            else:
                self.hyper_map(
                    F.partial(_run_optim_adamw_opt, self.adamw_opt, beta1,
                              beta2, float(lr), group.get("eps"),
                              group.get("weight_decay"), self.state_step,
                              group.get("amsgrad"), maximize),
                    self.master_parameters[start_id:end_id], grads,
                    self.exp_avg[start_id:end_id],
