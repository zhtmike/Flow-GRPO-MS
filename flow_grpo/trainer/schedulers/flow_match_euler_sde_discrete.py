...
class FlowMatchEulerSDEDiscreteScheduler(FlowMatchEulerDiscreteScheduler):
    ...
    def step(
        ...
        if isinstance(
                timestep,
                int) or (isinstance(timestep, ms.Tensor)
        ...
        prev_sample = prev_sample_mean + mint.sqrt(
            -var_t * dt) * variance_noise
    ...
    def compute_prev_sample_mean(
            self, model_output: ms.Tensor, sample: ms.Tensor, sigma: ms.Tensor,
        ...
        var_t = (0.7**
                 2) * sigma / (1 - mint.where(sigma == 1, sigma_max, sigma))
