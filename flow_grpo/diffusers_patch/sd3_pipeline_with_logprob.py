...
from mindone.diffusers.pipelines.stable_diffusion_3.pipeline_output import \
...
def pipeline_with_logprob(
    ...
    if (ip_adapter_image is not None and
            self.is_ip_adapter_active) or ip_adapter_image_embeds is not None:
    ...
    with self.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            if self.do_classifier_free_guidance:
                ...
                should_skip_layers = (
                    True if i > num_inference_steps * skip_layer_guidance_start
                    and i < num_inference_steps * skip_layer_guidance_stop else
                    False)
    ...
    return StableDiffusion3PipelineOutput(
        images=image), all_latents, all_log_probs, all_kl
